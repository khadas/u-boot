...
#define CONFIG_EXTRA_ENV_SETTINGS \
	...
	"get_os_type=if store read ${os_ident_addr} ${boot_part} 0 0x1000;"\
		" then os_ident ${os_ident_addr}; fi\0"\
	"fatload_dev=usb\0"\
	...
	"storeargs="\
		...
		"vout=${outputmode},enable panel_type=${panel_type} "\
	...
	"storeboot="\
		...
		"else if test ${os_type} = kernel; then "\
			"if fdt addr ${dtb_mem_addr}; "\
			"then else echo retry common dtb; run common_dtb_load; fi;"\
	...
	/* third ext-sd autoscr/recovery,last udisk autoscr/recovery */ \
		"run usb_burning; "\
	...
	"recovery_from_fat_dev="\
		"setenv loadaddr ${loadaddr_kernel};"\
		"if fatload ${fatload_dev} 0 ${loadaddr} aml_autoscript; "\
		"then autoscr ${loadaddr}; fi;"\
		"if fatload ${fatload_dev} 0 ${loadaddr} recovery.img; then "\
			"if fatload ${fatload_dev} 0 ${dtb_mem_addr} dtb.img; "\
			"then echo ${fatload_dev} dtb.img loaded; fi;"\
			"bootm ${loadaddr};fi;"\
		"\0"\
	"recovery_from_udisk="\
