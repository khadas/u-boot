...
/*
 * board/amlogic/configs/t5d_am319_v1.h
 *
...
#define CONFIG_VCCK_INIT_VOLTAGE	1030		// VCCK power up voltage
...
#define CONFIG_EXTRA_ENV_SETTINGS \
	...
	"disable_ir=0\0"\
        ...
        "storeargs="\
            ...
            "setenv bootargs ${initargs} otg_device=${otg_device} logo=${display_layer},loaded,${fb_addr} powermode=${powermode} fb_width=${fb_width} fb_height=${fb_height} display_bpp=${display_bpp} outputmode=${outputmode} vout=${outputmode},enable panel_type=${panel_type} lcd_ctrl=${lcd_ctrl} lcd_debug=${lcd_debug} hdmimode=${hdmimode} cvbsmode=${cvbsmode} osd_reverse=${osd_reverse} video_reverse=${video_reverse} androidboot.selinux=${EnableSelinux} androidboot.firstboot=${firstboot} jtag=${jtag} disable_ir=${disable_ir};"\
        ...
        "cmdline_keys="\
            "if keyman init 0x1234; then "\
                "if keyman read usid ${loadaddr} str; then "\
                    ...
                    "setenv serial ${usid};"\
                "else "\
					"setenv bootargs ${bootargs} androidboot.serialno=am319${cpu_id};"\
					"setenv serial am319${cpu_id};"\
                "fi;"\
