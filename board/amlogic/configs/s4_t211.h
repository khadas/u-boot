	...
	"lock=10101000\0"\
		...
		"cmdline_keys="\
			...
			"if keyman read mac_wifi ${loadaddr} str; then "\
				...
				"setenv bootconfig ${bootconfig} androidboot.mac_wifi=${mac_wifi};"\
			"else "\
				"setenv bootargs ${bootargs} mac_wifi=00:11:22:33:44:55;"\
				"setenv bootconfig ${bootconfig} "\
				"androidboot.mac_wifi=00:11:22:33:44:55;"\
			"fi;"\
			"if keyman read mac_bt ${loadaddr} str; then "\
				"setenv bootargs ${bootargs} mac_bt=${mac_bt};"\
				"setenv bootconfig ${bootconfig} androidboot.mac_bt=${mac_bt};"\
			"fi;"\
			"fi;"\
            "\0"\
        "upgrade_key="\
