...
/*
 * board/amlogic/configs/t5_ak301_v1.h
 *
...
#define CONFIG_EXTRA_ENV_SETTINGS \
	...
	"boot_flag=0\0"\
		...
		"ramoops.console_size=0x4000 loop.max_part=4 cramble_reg=0xff6345c4 "\
            "\0"\
        "upgrade_check="\
        ...
        "storeargs="\
            ...
            "setenv bootargs ${initargs} otg_device=${otg_device} logo=${display_layer},loaded,${fb_addr} powermode=${powermode} fb_width=${fb_width} fb_height=${fb_height} display_bpp=${display_bpp} outputmode=${outputmode} vout=${outputmode},enable panel_name=${panel_name} panel_type=${panel_type} lcd_ctrl=${lcd_ctrl} lcd_debug=${lcd_debug} hdmimode=${hdmimode} cvbsmode=${cvbsmode} osd_reverse=${osd_reverse} video_reverse=${video_reverse} androidboot.selinux=${EnableSelinux} androidboot.firstboot=${firstboot} jtag=${jtag} mem_size=${mem_size} irq_check_en=${Irq_check_en} ; "\
        ...
        "cmdline_keys="\
            "if keyman init 0x1234; then "\
                "if keyman read usid ${loadaddr} str; then "\
                    ...
                    "setenv serial ${usid};"\
                "else "\
					"setenv bootargs ${bootargs} androidboot.serialno=ak301${cpu_id};"\
					"setenv serial ak301${cpu_id};"\
                "fi;"\
...
#define CONFIG_DEVICE_PRODUCT	"t9623_ak301"
...
#define CONFIG_AML_V3_FACTORY_BURN              1       //support factory usb burning
...
/*LED driver*/
#define CONFIG_MESON_LEDS_STATE_CONTROL
