...
#define CONFIG_ROOTFS_TYPE "rootfstype=ramfs"
#define CONFIG_DTB_LOAD_AB  "imgread dtb ${boot_part} ${dtb_mem_addr}"
...
#define CONFIG_EXTRA_ENV_SETTINGS \
		...
		"bcb_cmd="\
			...
			"if test ${active_slot} != normal; then "\
				...
				"setenv common_dtb_load " CONFIG_DTB_LOAD_AB ";"\
			"fi;"\
			"\0"\
		"load_bmp_logo="\
			"if rdext4pic ${board_logo_part} $loadaddr; then "\
			"bmp display $logoLoadAddr; " \
		...
		"cmdline_keys="\
			...
			"if keyman init 0x1234; then "\
				...
				"else "\
					"setenv bootargs ${bootargs} "\
					"androidboot.serialno=an400${cpu_id};"\
					"setenv serial an400${cpu_id}; "\
					"setenv serial# an400${cpu_id};"\
				"fi;"\
