...
static int sd_emmc_init(unsigned int port)
{
	switch (port) {
	case SDIO_PORT_A:
		break;
...
extern unsigned int sd_debug_board_1bit_flag;

static void sd_emmc_pwr_prepare(unsigned int port)
{
	cpu_sd_emmc_pwr_prepare(port);
}

static void sd_emmc_pwr_on(unsigned int port)
{
	switch (port) {
	case SDIO_PORT_A:
		break;
...
static void sd_emmc_pwr_off(unsigned int port)
{
	/// @todo NOT FINISH
	switch (port) {
	case SDIO_PORT_A:
...
// #define CONFIG_TSD	  1
static void board_mmc_register(unsigned int port)
{
