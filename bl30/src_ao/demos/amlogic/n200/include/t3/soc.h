...
/* define power and corner informainton */
#define VAD_FUNC_MASK		0xf
#define VAD_FUNC_OFFSET		20
#define CHIP_CORNER_MASK	0xf
#define CHIP_CORNER_OFFSET	16
#define SUSPEND_FLAG_MASK	0xffff
#define SUSPEND_FLAG_OFFSET	0
#define SUSPEND_MSG(c, f, v)	\
	(((c) & CHIP_CORNER_MASK) << CHIP_CORNER_OFFSET | \
	 ((f) & SUSPEND_FLAG_MASK) << SUSPEND_FLAG_OFFSET | \
	 ((v) & VAD_FUNC_MASK) << VAD_FUNC_OFFSET)

#define GET_CORNER(v) \
	(((v) >> CHIP_CORNER_OFFSET) & CHIP_CORNER_MASK)

#define GET_SUSPEND_FLAG(v) \
	(((v) >> SUSPEND_FLAG_OFFSET) & SUSPEND_FLAG_MASK)

#define GET_VAD_INFO(v) \
	(((v) >> VAD_FUNC_OFFSET) & VAD_FUNC_MASK)
