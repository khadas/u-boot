	...
	if (bootm_argv[3]) {
		...
		if (fdtfile) {
			...
			if (!strcmp(env_get("mipi_lcd_exist"), "0")) {
				...
				run_command("fdt addr ${fdt_addr_r}; fdt resize 65536; fdt set /lcd status disabled;fdt set /lcd1 status okay;fdt set /lcd2 status disabled", 0);
			} else if (!strcmp(env_get("mipi_lcd_exist"), "1")) {
