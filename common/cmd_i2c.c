				...
				char * const argv[])
{
	...
	ret = i2c_get_chip_for_busnum(bus_no, 0x52, &dev);
		if (ret) {
	...
	if (argc == 1) {
		/* read fd/fe/ff */
		ret = i2c_read(dev, 0xfd, &buf[0], 1);
		ret = i2c_read(dev, 0xfe, &buf[1], 1);
		ret = i2c_read(dev, 0xff, &buf[2], 1);
		if (ret) {
		...
		for (i = 0; i < 3; i++) {
			ret = i2c_write(dev, 0xfd + i, &buf[i], 1);
			if (ret)
				return ret;
