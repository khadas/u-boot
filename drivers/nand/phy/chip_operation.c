...
int amlnand_store_interf_init(struct amlnand_chip *aml_chip)
{
	struct store_operation *storage_opera =
		&(aml_chip->storage_opera);

	if (!storage_opera->get_part_size)
		storage_opera->get_part_size = amlnf_get_size;

	if (!storage_opera->read)
		storage_opera->read = amlnf_read;

	if (!storage_opera->write)
		storage_opera->write = amlnf_write;

	if (!storage_opera->erase)
		storage_opera->erase = amlnf_erase;

	if (!storage_opera->get_copies)
		storage_opera->get_copies = amlnf_boot_cpys;

	if (!storage_opera->get_copy_size)
		storage_opera->get_copy_size = amlnf_boot_copy_size;

	if (!storage_opera->boot_read)
		storage_opera->boot_read = amlnf_boot_read;

	if (!storage_opera->boot_write)
		storage_opera->boot_write = amlnf_boot_write;

	if (!storage_opera->boot_erase)
		storage_opera->boot_erase = amlnf_boot_erase;

	if (!storage_opera->get_rsv_size)
		storage_opera->get_rsv_size = amlnf_get_rsv_size;

	if (!storage_opera->read_rsv)
		storage_opera->read_rsv = amlnf_read_rsv;

	if (!storage_opera->write_rsv)
		storage_opera->write_rsv = amlnf_write_rsv;

	if (!storage_opera->erase_rsv)
		storage_opera->erase_rsv = amlnf_erase_rsv;

	if (!storage_opera->protect_rsv)
		storage_opera->protect_rsv = amlnf_rsv_protect;
