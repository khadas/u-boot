/*
 * (C) Copyright 2021 Shenzhen Wesion Co., Ltd
 *
...
static struct udevice *fusb302_i2c_cur_bus;
...
static int fusb302_i2c_set_bus_num(unsigned int busnum)
{
	struct udevice *bus;
	int ret;
	...
	fusb302_i2c_cur_bus = bus;

	return 0;
}

static int fusb302_i2c_get_cur_bus(struct udevice **busp)
{
	if (!fusb302_i2c_cur_bus) {
		if (fusb302_i2c_set_bus_num(FUSB302_I2C_BUS_NUM)) {
	...
	*busp = fusb302_i2c_cur_bus;

	return 0;
}


static int fusb302_i2c_get_cur_bus_chip(uint chip_addr, struct udevice **devp)
{
	struct udevice *bus;
	int ret;

	ret = fusb302_i2c_get_cur_bus(&bus);
	if (ret)
	return ret;
...
{
	return fusb302_i2c_set_bus_num(FUSB302_I2C_BUS_NUM);
}
...
int fusb302_i2c_read(u8 reg, u8 *val)
{
	...
	ret = fusb302_i2c_get_cur_bus_chip(chip.addr, &dev);
...
//	ret = i2c_read(chip.addr, reg, 1, val, val_len);
	...
	ret = fusb302_i2c_get_cur_bus_chip(chip.addr, &dev);
	if (!ret)
		ret = dm_i2c_read(dev, reg, val, val_len);
	if (ret != 0)
		printf("fusb302 i2c read error!!!\n");
...
int fusb302_init(void)
{
	...
	ret = fusb302_i2c_probe(6, chip.addr);
	if (ret) {
