...
static struct iw7027_he *bl_iw7027_he;
...
*/
static int iw7027_he_wreg(struct spi_slave *spi,
			  unsigned char addr,
			  unsigned char val)
{
	...
	tbuf[0] = BROADCAST_MSG | SINGLE_DATA;
	tbuf[1] = addr & 0x7f;
	tbuf[2] = val;
	tbuf[3] = 0x00;
...
static int iw7027_he_wreg_MultipleByte(struct spi_slave *spi,
				       unsigned char addr,
				       unsigned char *val,
				       int len)
{
/* iw7025 be used pwm1-pwm12,register 0x21-0x32, 18byte, 108*1.5+8+3 data*/
/* DevAddr = 0xBF, NrOfByte = 0x12, addr = 0x21*/
	...
	val[0] = BROADCAST_MSG | BLOCK_DATA;
	val[1] = len;
	val[2] = addr & 0x7f;
	val[len+3] = 0x00;
	ret = ldim_spi_write(spi, val, len + 4);

	return ret;
}
#if 0
static int iw7027_he_wreg_MulDifData(struct spi_slave *spi,
				     unsigned char addr,
				     unsigned char *val,
				     int len)
{
...
static int iw7027_he_rreg(struct spi_slave *spi, unsigned char DevAddr,
			  unsigned char addr,
			  unsigned char *val)
{
	...
	for (i = 0; i < 2; i++)
		tbuf[i+2] = 0x00;
	ret = ldim_spi_read(spi, tbuf, 4, rbuf, 1);
	*val = rbuf[3];
...
static int iw7027_he_hw_init_on(void)
{
	...
	for (spi_device_id = 0; spi_device_id < device_count;
	     spi_device_id++) {
		iw7027_he_wreg(bl_iw7027_he->spi, spi_device_id, 0x06);
		for (i = 0; i <= 10; i++) {
			reg_chk = 0;
			iw7027_he_rreg(bl_iw7027_he->spi, spi_device_id,
				       0x00, &reg_chk);
			if (reg_chk == 0x06)
				break;
			if (i == 10) {
	...
	while (++i < 100) {
		for (spi_device_id = 0; spi_device_id < device_count;
		     spi_device_id++) {
			iw7027_he_rreg(bl_iw7027_he->spi, spi_device_id, 0xb3,
				       &reg_duty_chk);
...
static unsigned int iw7027_he_get_value(unsigned int level)
{
	unsigned int val;
...
static int iw7027_he_smr(unsigned short *buf, unsigned char len)
{
	...
	iw7027_he_wreg_MultipleByte(bl_iw7027_he->spi, 0x40,
				    val_brightness, (num * 2));
...
int ldim_dev_iw7027_he_probe(struct aml_ldim_driver_s *ldim_drv)
{
	...
	bl_iw7027_he = (struct iw7027_he *)malloc(sizeof(struct iw7027_he));
	if (bl_iw7027_he == NULL) {
		LDIMERR("iw7027_he malloc error\n");
		return -1;
	}
	memset(bl_iw7027_he, 0, sizeof(struct iw7027_he));

	iw7027_he_on_flag = 0;

	bl_iw7027_he->spi = ldim_drv->spi_info->spi;
