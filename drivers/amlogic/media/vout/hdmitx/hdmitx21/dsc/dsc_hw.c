...
#define DSC_PHY_ADDR_BASE 0xfe0be000

static int dsc_reg_dbg;

u32 R_DSC_REG(u32 reg)
{
	u32 val;

	val = readl(DSC_PHY_ADDR_BASE + (reg << 2));
...
void W_DSC_REG(u32 reg, u32 val)
{
	u32 rval;
	u32 paddr = DSC_PHY_ADDR_BASE + (reg << 2);
...
u32 dsc_vcbus_reg_read(u32 _reg)
{
	return hd21_read_reg(_reg);
};

void dsc_vcbus_reg_write(u32 _reg, u32 _value)
{
	return hd21_write_reg(_reg, _value);
};

void dsc_vcbus_reg_setb(u32 reg, u32 value,
		       u32 _start, u32 _len)
{
...
unsigned int dsc_vcbus_reg_getb(u32 reg,
			       u32 _start, u32 _len)
{
...
#ifndef P_RESETCTRL_RESET0
#define P_RESETCTRL_RESET0 0xfe002000
#endif
/* TODO, add protection only for S5 */
void dsc_enc_rst(void)
{
	/* If set RESET_reg = 1, will reset the module. RESET_reg is an
	 * auto-recover register which will recover to 0 after some cycles.
	 */
	writel(BIT(30), P_RESETCTRL_RESET0);
}
