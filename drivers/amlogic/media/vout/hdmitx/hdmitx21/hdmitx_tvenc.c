...
static void _set_vmode_enc_hw(const struct reg_s *s)
{
	if (!s)
		return;
...
static void hdmi_tvenc1080i_set(enum hdmi_vic vic)
{
	...
	case HDMI_46_1920x1080i120_16x9:
		...
		BACK_PORCH = 148;
		VSYNC_LINES = 5;
		_set_vmode_enc_hw(tvregs_1080i60);
		break;
	...
	case HDMI_40_1920x1080i100_16x9:
		...
		BACK_PORCH = 148;
		VSYNC_LINES = 5;
		_set_vmode_enc_hw(tvregs_1080i50);
		break;
	default:
		break;
	}
...
static void config_tv_enci(enum hdmi_vic vic)
{
	...
	case HDMI_59_720x480i240_16x9:
		...
		/* note below two sentence are copied from 576i
		 * otherwise, the line count for 480i is wrong
		 */
		hd21_write_reg(ENCI_VIDEO_MODE, 0x8); /* Set for 480I mode */
		/* Set for High Bandwidth for CBW & YBW */
		hd21_write_reg(ENCI_VIDEO_MODE_ADV, 0x26);
