...
static int dptx_aux_check(int index)
{
	if (dptx_reg_read(index, EDP_TX_TRANSMITTER_OUTPUT_ENABLE))
		return 0;
...
static void dptx_aux_request(int index, struct dptx_aux_req_s *req)
{
	...
	dptx_reg_write(index, EDP_TX_AUX_ADDRESS, req->address);
	/*submit data only for write commands*/
	if (req->cmd_state == 0) {
		for (i = 0; i < req->byte_cnt; i++)
			dptx_reg_write(index, EDP_TX_AUX_WRITE_FIFO, req->data[i]);
	}
	/*submit the command and the data size*/
	dptx_reg_write(index, EDP_TX_AUX_COMMAND,
		       ((req->cmd_code << 8) | ((req->byte_cnt - 1) & 0xf)));
}

static int dptx_aux_submit_cmd(int index, struct dptx_aux_req_s *req)
{
	unsigned int status = 0, reply = 0;
	unsigned int retry_cnt = 0, timeout = 0;
	char str[8];

	if (dptx_aux_check(index))
		return -1;
...
dptx_aux_submit_cmd_retry:
	...
	if (retry_cnt++ < EDP_AUX_RETRY_CNT) {
		...
		LCDPR("[%d]: edp aux %s addr 0x%x timeout, status 0x%x, reply 0x%x, retry %d\n",
		      index, str, req->address, status, reply, retry_cnt);
...
static int dptx_aux_write(int index, unsigned int addr, unsigned int len, unsigned char *buf)
{
	...
	ret = dptx_aux_submit_cmd(index, &aux_req);
	return ret;
}
...
static int dptx_aux_i2c_read(int index, unsigned int dev_addr,
			     ...
			     unsigned char *buf)
{
	...
	while (n < len) {
		...
		reply_count = dptx_reg_read(index, EDP_TX_AUX_REPLY_DATA_COUNT);
		for (i = 0; i < reply_count; i++) {
			buf[n] = dptx_reg_read(index, EDP_TX_AUX_REPLY_DATA);
			n++;
		}
...
static void dptx_edid_print(struct dptx_edid_s *edp_edid)
{
	...
	if (edp_edid->string_flag & (1 << 1))
		printf("Monitor AScii String:  %s\n", edp_edid->asc_string);
	if (edp_edid->string_flag & (1 << 2))
		printf("Monitor SN:            %s\n", edp_edid->serial_num);

	printf("Detail Timing:\n"
		"    Pixel Clock:   %d.%dMHz\n"
		"    H Active:      %d\n"
		"    H Blank:       %d\n"
		"    V Active:      %d\n"
...
static int dptx_read_edid(int index, unsigned char *edid_buf)
{
	int i, ret;
	...
	for (i = 0; i < 128; i += 16) {
		ret = dptx_aux_i2c_read(index, 0x50, i, 16, &edid_buf[i]);
		if (ret)
			break;
	}
...
int dptx_edid_dump(struct aml_lcd_drv_s *pdrv)
{
	...
	edid_buf = (unsigned char *)malloc(128 * sizeof(unsigned char));
	if (!edid_buf)
		return -1;

dptx_edid_dump_retry:
	memset(edid_buf, 0, 128 * sizeof(unsigned char));
	ret = dptx_read_edid(pdrv->index, edid_buf);
	if (ret) {
	...
	dptx_edid_phrase(edid_buf, &edp_edid);
...
dptx_edid_timing_probe_retry:
		memset(edid_buf, 0, 128 * sizeof(unsigned char));
		ret = dptx_read_edid(index, edid_buf);
		if (ret) {
			if (retry_cnt++ > DPTX_EDID_READ_RETRY_MAX) {
				LCDERR("[%d]: %s: failed to read EDID from sink, retry %d\n",
				       index, __func__, retry_cnt);
