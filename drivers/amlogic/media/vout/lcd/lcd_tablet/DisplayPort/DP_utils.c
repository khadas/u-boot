...
{
	enum DPCD_level_e DPstd_vswing_level;

	if (phy_vswing <= 0x3)
		DPstd_vswing_level = VAL_DP_std_LEVEL_0;
	else if (phy_vswing <= 0x7)
		DPstd_vswing_level = VAL_DP_std_LEVEL_1;
	else if (phy_vswing <= 0xb)
		DPstd_vswing_level = VAL_DP_std_LEVEL_2;
	else if (phy_vswing <= 0xf)
		DPstd_vswing_level = VAL_DP_std_LEVEL_3;
	else
		DPstd_vswing_level = VAL_DP_std_LEVEL_0;

	return DPstd_vswing_level;
}

enum DPCD_level_e dptx_preem_phy_to_std(unsigned int phy_preem)
{
	enum DPCD_level_e DPstd_preem_level;

	if (phy_preem <= 0x3)
		DPstd_preem_level = VAL_DP_std_LEVEL_0;
	else if (phy_preem <= 0x7)
		DPstd_preem_level = VAL_DP_std_LEVEL_1;
	else if (phy_preem <= 0xb)
		DPstd_preem_level = VAL_DP_std_LEVEL_2;
	else if (phy_preem <= 0xf)
		DPstd_preem_level = VAL_DP_std_LEVEL_3;
	else
		DPstd_preem_level = VAL_DP_std_LEVEL_0;

	return DPstd_preem_level;
}
