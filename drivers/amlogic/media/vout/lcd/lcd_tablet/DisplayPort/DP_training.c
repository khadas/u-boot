...
static int dptx_training_phy_update_apply(struct aml_lcd_drv_s *pdrv)
{
	...
	dptx_reg_write(pdrv->index, EDP_TX_PHY_VOLTAGE_DIFF_LANE_0, max_vsw);
	dptx_reg_write(pdrv->index, EDP_TX_PHY_VOLTAGE_DIFF_LANE_1, max_vsw);
	dptx_reg_write(pdrv->index, EDP_TX_PHY_VOLTAGE_DIFF_LANE_2, max_vsw);
	dptx_reg_write(pdrv->index, EDP_TX_PHY_VOLTAGE_DIFF_LANE_3, max_vsw);
	dptx_reg_write(pdrv->index, EDP_TX_PHY_PRE_EMPHASIS_LANE_0, max_pre);
	dptx_reg_write(pdrv->index, EDP_TX_PHY_PRE_EMPHASIS_LANE_1, max_pre);
	dptx_reg_write(pdrv->index, EDP_TX_PHY_PRE_EMPHASIS_LANE_2, max_pre);
	dptx_reg_write(pdrv->index, EDP_TX_PHY_PRE_EMPHASIS_LANE_3, max_pre);
	...
	//dptx_reg_write(pdrv->index, EDP_TX_PHY_VOLTAGE_DIFF_LANE_0, _cfg->adj_req_vswing[0]);
	//dptx_reg_write(pdrv->index, EDP_TX_PHY_VOLTAGE_DIFF_LANE_1, _cfg->adj_req_vswing[1]);
	//dptx_reg_write(pdrv->index, EDP_TX_PHY_VOLTAGE_DIFF_LANE_2, _cfg->adj_req_vswing[2]);
	//dptx_reg_write(pdrv->index, EDP_TX_PHY_VOLTAGE_DIFF_LANE_3, _cfg->adj_req_vswing[3]);
	//dptx_reg_write(pdrv->index, EDP_TX_PHY_PRE_EMPHASIS_LANE_0, _cfg->adj_req_preem[0]);
	//dptx_reg_write(pdrv->index, EDP_TX_PHY_PRE_EMPHASIS_LANE_1, _cfg->adj_req_preem[1]);
	//dptx_reg_write(pdrv->index, EDP_TX_PHY_PRE_EMPHASIS_LANE_2, _cfg->adj_req_preem[2]);
	//dptx_reg_write(pdrv->index, EDP_TX_PHY_PRE_EMPHASIS_LANE_3, _cfg->adj_req_preem[3]);
...
static int dptx_set_training_pattern(struct aml_lcd_drv_s *pdrv, unsigned char pattern)
{
	...
	dptx_reg_write(pdrv->index, EDP_TX_TRAINING_PATTERN_SET, pattern);
...
static int dptx_run_training_loop(struct aml_lcd_drv_s *pdrv, unsigned int retry_cnt)
{
	...
	dptx_reg_write(pdrv->index, EDP_TX_SCRAMBLING_DISABLE, 0x01);
...
int dptx_fast_link_training(struct aml_lcd_drv_s *pdrv)
{
	...
	// disable scrambling
	dptx_reg_write(pdrv->index, EDP_TX_SCRAMBLING_DISABLE, 0x1);

	// set training pattern 1
	dptx_reg_write(pdrv->index, EDP_TX_TRAINING_PATTERN_SET, 0x1);
	p_data = 0x21;
	ret = dptx_aux_write(pdrv, DPCD_TRAINING_PATTERN_SET, 1, &p_data);
	if (ret)
	...
	dptx_reg_write(pdrv->index, EDP_TX_TRAINING_PATTERN_SET, 0x2);
	p_data = 0x22;
	ret = dptx_aux_write(pdrv, DPCD_TRAINING_PATTERN_SET, 1, &p_data);
	if (ret & 0x02)
	...
	dptx_reg_write(pdrv->index, EDP_TX_TRAINING_PATTERN_SET, 0x3);
	p_data = 0x23;
	ret = dptx_aux_write(pdrv, DPCD_TRAINING_PATTERN_SET, 1, &p_data);
	if (ret)
	...
	dptx_reg_write(pdrv->index, EDP_TX_TRAINING_PATTERN_SET, 0x0);
...
static int dptx_sink_handshake_check(struct aml_lcd_drv_s *pdrv)
{
	...
	if (ret) {
		LCDPR("[%d]: %s: read DPCD_MAX_DOWNSPREAD failed\n", pdrv->index, __func__);
		return -1;
	}
	...
	if (lcd_debug_print_flag & LCD_DBG_PR_NORMAL)
		LCDPR("[%d]: %s: DPCD(0x%x)[6] = %d\n",
			pdrv->index, __func__, DPCD_MAX_DOWNSPREAD, data);
	return data;
}
