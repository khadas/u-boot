...
/*
 * drivers/mtd/nand/amlogic_mtd/boot.c
 *
...
int get_boot_num(struct mtd_info *mtd, size_t rwsize)
{
	...
	available_space = mtd->size - skip - 2 * mtd->writesize;
...
void nand_info_page_prepare(struct aml_nand_chip *aml_chip, u8 *page0_buf)
{
	...
	ddrp_start_block = aml_chip_normal->aml_nandddr_info->valid_node->phy_blk_addr;
