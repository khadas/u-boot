...
static inline void meson_rsv_disprotect(void)
{
	rsv_protect = 0;
}

static inline void meson_rsv_protect(void)
{
	rsv_protect = 1;
}

static inline int meson_rsv_isprotect(void)
{
	return rsv_protect;
}
			    ...
			    u32 block_addr)
{
	...
	if (handler->bbt && handler->bbt->valid) {
		if (block_addr >= handler->bbt->start &&
...
int meson_rsv_erase(struct meson_rsv_info_t *rsv_info)
{
	...
	pr_info("%s %d rsv erasing %s\n",
		__func__, __LINE__, rsv_info->name);
	erase_info.mtd = mtd;
	erase_info.addr = rsv_info->start * mtd->erasesize;
	erase_info.len = (rsv_info->end - rsv_info->start) *
					mtd->erasesize;
	meson_rsv_disprotect();
	ret = mtd_erase(mtd, &erase_info);
	meson_rsv_protect();
...
RE_RSV_INFO:
	...
	pr_info("%s blk = %d, ec = %d, page = %d, timestamp = %d\n",
		rsv_info->name, rsv_info->nvalid->ec,
		rsv_info->nvalid->blk_addr,
		   ...
		   struct meson_rsv_handler_t *handler)
{
	...
	rsv_handler = handler;
	pr_info("bbt_start=%d ", handler->bbt->start);
#ifndef CONFIG_ENV_IS_IN_NAND
	pr_info("env_start=%d ", handler->env->start);
#endif
	pr_info("key_start=%d ", handler->key->start);
	pr_info("dtb_start=%d ", handler->dtb->start);
	pr_info("\n");
...
#if 0
static int meson_rsv_bbt_write(u_char *source, size_t size)
{
	u_char *temp;
	size_t len;
	int ret;
...
#if 0
static int meson_rsv_bbt_erase(void)
{
	if (!rsv_handler ||
	    !rsv_handler->bbt) {
		pr_info("%s %d rsv info has not inited yet!\n",
