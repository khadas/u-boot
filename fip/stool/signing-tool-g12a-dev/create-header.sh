...
add_aml_block_header() {
    ...
    # Verify args
    if [[ -n "$keylen" ]]; then
        echo Error: not yet implemented keylen=$keylen
        exit 1
    fi
    check_file input "$input"
    local filesize=$(wc -c < "$input")
    if [[ $filesize -ne $(( $bl2size - 4*1024 )) ]]; then
        echo Error: invalid bl payload size
        exit 1
    fi
    ...
    # Create header
    head -c $((4096 - 16)) /dev/zero > "$output"
    write_uint32_le "$output" $((0x4c4d4140)) 0
    write_uint8 "$output" 64 8      # bySizeHdr
    write_uint8 "$output" 1  10     # byVerMajor
    write_uint8 "$output" 1  11     # byVerMinor

    write_uint32_le "$output" $(( $bl2size - 16 )) 4    # nTotalSize = bl2size - sizeof(iv)

    write_uint32_le "$output" $((0x40))  20    # nSigOffset
    write_uint32_le "$output" $((0x200)) 24    # nSigLen
    write_uint32_le "$output" $((0x60))  28    # nCHKStart
    write_uint32_le "$output" $((0x240)) 36    # nPUKOffset
    write_uint32_le "$output" $((0xdb0)) 40    # nPUKDataLen

    write_uint32_le "$output" $(( $bl2size - 16 - 64 - 32 )) 44    # nCHKSize

    write_uint32_le "$output" $((0xff0)) 52    # nDataOffset
    write_uint32_le "$output" $(( $bl2size - 4 * 1024 )) 56    # nDataLen

    if [[ $(wc -c < "$output") -ne $((4096 - 16)) ]]; then
        echo Error: Internal error h-u-1
        exit 1
    fi

    cat "$input" >> "$output"

    if [[ $(wc -c < "$output") -ne $(( $bl2size - 16 )) ]]; then
        echo Error: Internal error h-u-2
        exit 1
    fi
}
    ...
    while [ $i -lt $# ]; do
        ...
        case "$arg" in
            ...
            --create-bl2-header)
                ...
                break ;;
            --add-aml-block-header)
                add_aml_block_header "${argv[@]:$((i + 1))}"
                break ;;
            *)
