...
uint32_t trusty_write_toybrick_seed(uint32_t *buf)
{
	...
	SharedMem0.size = 3 * sizeof(uint32_t);
	SharedMem0.flags = 0;
...
uint32_t trusty_read_toybrick_SnMacAc(uint8_t *data, uint32_t size)
{
	...
	SharedMem0.size = sizeof("toybrick_SnMacAc");
	SharedMem0.flags = 0;
	...
	memcpy(SharedMem0.buffer, "toybrick_SnMacAc", SharedMem0.size);
...
uint32_t trusty_write_toybrick_extrakey(uint8_t *data, uint32_t size)
{
	...
	SharedMem0.size = sizeof("toybrick_extrakey");
	SharedMem0.flags = 0;
	...
	memcpy(SharedMem0.buffer, "toybrick_extrakey", SharedMem0.size);
