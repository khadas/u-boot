...
void hd_set_reg_bits(unsigned int addr, unsigned int value, unsigned int offset, unsigned int len)
{
...
unsigned int hdmitx_rd_reg(unsigned int addr)
{
	...
	switch (large_offset) {
	case 0x10:
		 /*DWC*/ hdmitx_addr = HDMITX_DWC_BASE_OFFSET + small_offset;
...
void hdmitx_wr_reg(unsigned int addr, unsigned int data)
{
	...
	case 00:
	default:
			/*TOP*/ if ((small_offset >= 0x2000) && (small_offset <= 0x365E)) {
			hdmitx_addr = HDMITX_TOP_BASE_OFFSET + small_offset;
			writeb(data & 0xff, hdmitx_addr);
		} else {
			hdmitx_addr = HDMITX_TOP_BASE_OFFSET + (small_offset << 2);
...
unsigned int hdmitx_rd_check_reg(unsigned int addr, unsigned int exp_data, unsigned int mask)
{
	return 0;
}
